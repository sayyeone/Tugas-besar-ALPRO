...
func editDataSiswa() {
	...
	var siswaIdx int
	const maxRetry = 3
	var found bool // Deklarasi variabel found
	...
	for attempt < maxRetry && !found {
		...
		if siswaIdx != -1 {
			found = true // loop berhenti otomatis
		} else {
			attempt++
			fmt.Printf("❌ Siswa dengan ID %d tidak ditemukan!\n", idSiswa)
		}
	...
	fmt.Scan(&input)
	if strings.TrimSpace(input) != "" && input != "-" {
		for input != "-" && !isValidDate(input) {
			fmt.Print("❌ Format tanggal salah! Gunakan format dd/mm/yyyy atau '-' untuk skip: ")
			fmt.Scan(&input)
		}
		if input != "-" {
